...
/*            Map<String, String> externalIds = sw360Release.getExternalIds();

            externalIds.entrySet().stream().forEach(entry -> {
                        String key = entry.getKey();
                        if (SW360CoordinateKeysToArtifactCoordinates.containsValue(key)) {
                            Class<? extends ArtifactCoordinates> currentClass = SW360CoordinateKeysToArtifactCoordinates
                                    .get(key);

                            //artifact.addFact(new currentClass.getEnclosingClass());
                            //getSw360KeysToAntennaClasses().get(entry.getKey().getClass())(entry.getValue()));
                        } else if (SW360KeysToAntennaClasses.getSw360KeysToAntennaClasses().containsKey(key)) {
                            Class<? extends  ArtifactFact> currentCoordinateClass = SW360KeysToAntennaClasses.getSw360KeysToAntennaClasses().get(key);
                        }
                    }); */
            ...
            mapSWHRepo(sw360Release, artifact);
            mapHashes(sw360Release, artifact);
            mapChangeStatus(sw360Release, artifact);
            // TODO add mapCopyrights here, once decided how to
        }
    }
    ...
    private void mapSWHRepo(SW360Release sw360Release, Artifact artifact) {
        if (sw360Release.getExternalIds().containsKey(SW360Attributes.RELEASE_EXTERNAL_ID_SWHREPO)) {
            artifact.addFact(new ArtifactSoftwareHeritageURL(sw360Release.getExternalIds().get(SW360Attributes.RELEASE_EXTERNAL_ID_SWHREPO)));
        }
    }
    ...
    private void mapObservedLicense(SW360Release sw360Release, Artifact artifact) {
        if (sw360Release.getExternalIds().containsKey(SW360Attributes.RELEASE_EXTERNAL_ID_OLICENSES)) {
            Optional<LicenseInformation> licenseInformation = artifact.askForGet(ObservedLicenseInformation.class);

            List<SW360SparseLicense> sw360SparseLicenses =
                    getSW360SparseLicensesFromString(sw360Release.getExternalIds().get(SW360Attributes.RELEASE_EXTERNAL_ID_OLICENSES), ";");

            List<License> artifactObservedLicenses = getLicenseListWithSW360AndArtifactLicenses(sw360SparseLicenses, licenseInformation);

            artifact.addFact(new ObservedLicenseInformation(LicenseSupport.mapLicenses(
                    artifactObservedLicenses.stream()
                            .map(License::getName)
    ...
    private void mapDeclaredLicense(SW360Release sw360Release, Artifact artifact) {
        if (sw360Release.getExternalIds().containsKey(SW360Attributes.RELEASE_EXTERNAL_ID_DLICENSES)) {
            Optional<LicenseInformation> licenseInformation = artifact.askForGet(DeclaredLicenseInformation.class);

            List<SW360SparseLicense> sw360SparseLicenses =
                    getSW360SparseLicensesFromString(sw360Release.getExternalIds().get(SW360Attributes.RELEASE_EXTERNAL_ID_DLICENSES), ";");

            List<License> artifactDeclaredLicenses = getLicenseListWithSW360AndArtifactLicenses(sw360SparseLicenses, licenseInformation);

            artifact.addFact(new DeclaredLicenseInformation(LicenseSupport.mapLicenses(
                    artifactDeclaredLicenses.stream()
                            .map(License::getName)
                            .collect(Collectors.toList())
            )));
        }
    ...
    private List<License> getLicenseListWithSW360AndArtifactLicenses(List<SW360SparseLicense> sw360SparseLicenses, Optional<LicenseInformation> licenseInformation) {
        List<License> artifactLicenses;
        if (licenseInformation.isPresent()) {
            artifactLicenses = licenseInformation.get().getLicenses();
        } else {
            artifactLicenses = new ArrayList<>();
        }

        if (hasDifferentLicenses(artifactLicenses, sw360SparseLicenses)) {
            for (SW360SparseLicense sw360SparseLicense : sw360SparseLicenses) {
                License tempLicense = new License();
                tempLicense.setName(sw360SparseLicense.getFullName());
                License license = makeLicenseFromLicenseDetails(new SW360License(tempLicense));
    ...
    private List<SW360SparseLicense> getSW360SparseLicensesFromString(String licenseString, String splitRegex) {
        String[] licenses = licenseString.split(splitRegex);
        List<SW360SparseLicense> sw360SparseLicenses = new ArrayList<>();

        for (String license : licenses) {
            SW360SparseLicense sw360SparseLicense = new SW360SparseLicense();
            sw360SparseLicenses.add(sw360SparseLicense.setFullName(license));
        }

        return sw360SparseLicenses;
    }
