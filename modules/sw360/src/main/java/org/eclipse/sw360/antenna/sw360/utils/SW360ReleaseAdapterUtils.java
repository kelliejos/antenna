...
public class SW360ReleaseAdapterUtils {
    ...
    public static void prepareRelease(SW360Release release, SW360Component component, Set<String> sw360LicenseIds, Artifact artifact) {
        ...
        SW360ReleaseAdapterUtils.setSwhRepo(release, artifact);
    ...
    private static void setDeclaredLicense(SW360Release release, Artifact artifact) {
        artifact.askForGet(DeclaredLicenseInformation.class)
                .ifPresent(licenseInformation -> release.setDeclaredLicense(licenseInformation.getLicenses().stream()
                        .map(License::getName)
    ...
    private static void setObservedLicense(SW360Release release, Artifact artifact) {
        artifact.askForGet(ObservedLicenseInformation.class)
                .ifPresent(licenseInformation -> release.setObservedLicense(licenseInformation.getLicenses().stream()
                        .map(License::getName)
                        .collect(Collectors.toSet()))
                );
    }
    ...
    private static void setSwhRepo(SW360Release release, Artifact artifact) {
        artifact.askForGet(ArtifactSoftwareHeritageURL.class)
                .ifPresent(release::setSoftwareHeritageUrl);
    }
